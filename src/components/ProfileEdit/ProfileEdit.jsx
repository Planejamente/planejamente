...
const [formacoes, setFormacoes] = useState([]); // [ { titulo: '', dataInicio: '', instituicao: '', dataFim: '', descricao: '' }
  ...
  const openModalB = () => setIsModalBOpen(true);
  const closeModalB = () => setIsModalBOpen(false);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const responseExp = await api.get(
          `/experiencias-formacoes/${Cookies.get("id")}`,
          {
            headers: {
              Authorization: `Bearer ${Cookies.get("token")}`,
            },
        ...
        const especialidades = await api.get(
          `/especialidades`,
          {
            headers: {
              Authorization: `Bearer ${Cookies.get("token")}`,
        ...
        const experiencias = responseExp.data.filter(exp => exp.tipo === "Experiência");
        const formacoes = responseExp.data.filter(exp => exp.tipo === "Formação");
  ...
  const handleSaveFormacao = () => {
    api
      ...
      .then((response) => {
        ...
        setReload(true);
        console.log("Formação salva com sucesso!", response);
      })
      .catch((error) => {
        // Tratar erro da requisição
        ...
        <>
          <div className={styles.main}>
            <div className={styles.formEditProfile}>
              ...
              <div className={styles.inputProfile}>
                ...
                <InputMod
                  ...
                  disabled={true}
                  onChange={(e) =>
                    setCnpj(mascara.formatarCnpj(e.target.value))
                  }
                />
                <InputMod
                  type="text"
            ...
            <div className={styles.containerOutrasInfos}>
              ...
              <div className={styles.formacoes}>
                ...
                <div>
                  <button>
                    <img src={BotaoOpenModal} alt="" onClick={openModalB}/>
                  </button>
                </div>
              </div>
              <div className={styles.especialidade}>
                ...
                <div>
                  <button>
                    <img src={BotaoOpenModal} alt="" onClick={openModalC}/>
                  </button>
                </div>
              </div>
            </div>
            ...
            >
              ...
              >
                <div className={styles.leftModal}>
                  <div className={styles.headerModal}>Suas Experiências</div>
                ...
                <div className={styles.rightModal}>
                  <div className={styles.expCards}>
                    {/* experiencias ai */}
                    {Array.isArray(exps) && exps.map((exp, index) => (
                      <div className={styles.expCard} key={index}>
                        <div className={styles.cargoExp}>
                          Cargo: {exp.cargo}
                        </div>
                        <div className={styles.instExp}>
                          Instituição: {exp.instituicao}
                        </div>
                        <div className={styles.dtInExp}>
                          Início: {exp.dataInicio}
                        </div>
                        <div className={styles.dtOutExp}>
                          Fim: {exp.dataFim}
                        </div>
          ...
          {isModalBOpen && (
            ...
            >
              ...
              >
                <div className={styles.leftModal}>
                  <div className={styles.headerModal}>Suas Formações</div>
                  <div className={styles.inputsModal}>
                    <div className={styles.sectionInputA}>
                      <InputMod
                        onChange={(e) => setCargo(e.target.value)}
                  ...
                  <div className={styles.btnSaveModal}>
                    <button onClick={handleSaveFormacao}>Salvar</button>
                  </div>
                </div>
                <div className={styles.rightModal}>
                  <div className={styles.expCards}>
                    {/* experiencias ai */}
                    {Array.isArray(formacoes) && formacoes.map((exp, index) => (
                      <div className={styles.expCard} key={index}>
                        <div className={styles.cargoExp}>
                          Título: {exp.cargo}
                        </div>
